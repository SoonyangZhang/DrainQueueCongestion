...
std::ostream& operator<<(std::ostream& os, const QuicSendTimeState& s) {
...
QuicByteCount MaxAckHeightTracker::Update(QuicBandwidth bandwidth_estimate,
  ...
  if (aggregation_epoch_start_time_ == ProtoTime::Zero()) {
...
QuicBandwidthSampler::QuicBandwidthSampler(
    ...
    : total_bytes_sent_(0),
      ...
      max_tracked_packets_(10000/*GetQuicFlag(FLAGS_quic_max_tracked_packet_count)*/),
...
QuicBandwidthSampler::QuicBandwidthSampler(const QuicBandwidthSampler& other)
...
void QuicBandwidthSampler::EnableOverestimateAvoidance() {
...
void QuicBandwidthSampler::OnPacketSent(
    ProtoTime sent_time,
  ...
  if (!connection_state_map_.IsEmpty() &&
    ...
    if (unacked_packet_map_ != nullptr) {
      /*QUIC_BUG << "QuicBandwidthSampler in-flight packet map has exceeded maximum "
  ...
  LOG_IF(FATAL,!success) << "QuicBandwidthSampler failed to insert the packet "
                           "into the map, most likely because it's already "
                           "in it.";
}

void QuicBandwidthSampler::OnPacketNeutered(QuicPacketNumber packet_number) {
...
QuicBandwidthSamplerInterface::CongestionEventSample
QuicBandwidthSampler::OnCongestionEvent(ProtoTime ack_time,
  ...
  QuicSendTimeState last_lost_packet_send_state;

  for (const LostPacket& packet : lost_packets) {
    QuicSendTimeState send_state =
  ...
  QuicSendTimeState last_acked_packet_send_state;
  for (const auto& packet : acked_packets) {
    QuicBandwidthSample sample =
...
QuicByteCount QuicBandwidthSampler::OnAckEventEnd(
...
QuicBandwidthSample QuicBandwidthSampler::OnPacketAcknowledged(
    ProtoTime ack_time,
    QuicPacketNumber packet_number) {
...
QuicBandwidthSample QuicBandwidthSampler::OnPacketAcknowledgedInner(
    ProtoTime ack_time,
    QuicPacketNumber packet_number,
...
bool QuicBandwidthSampler::ChooseA0Point(QuicByteCount total_bytes_acked,
                                     AckPoint* a0) {
...
QuicSendTimeState QuicBandwidthSampler::OnPacketLost(QuicPacketNumber packet_number,
                                             QuicPacketLength bytes_lost) {
  // TODO(vasilvv): see the comment for the case of missing packets in
  // QuicBandwidthSampler::OnPacketAcknowledged on why this does not raise a
  // QUIC_BUG when removal fails.
  QuicSendTimeState send_time_state;
...
void QuicBandwidthSampler::SentPacketToSendTimeState(
    const ConnectionStateOnSentPacket& sent_packet,
...
void QuicBandwidthSampler::RemoveObsoletePackets(QuicPacketNumber least_unacked) {
  ...
  // marked as acked or lost in the QuicBandwidthSampler.
  connection_state_map_.RemoveUpTo(least_unacked);
}

QuicByteCount QuicBandwidthSampler::total_bytes_sent() const {
  return total_bytes_sent_;
}

QuicByteCount QuicBandwidthSampler::total_bytes_acked() const {
  return total_bytes_acked_;
}

QuicByteCount QuicBandwidthSampler::total_bytes_lost() const {
  return total_bytes_lost_;
}

QuicByteCount QuicBandwidthSampler::total_bytes_neutered() const {
  return total_bytes_neutered_;
}

bool QuicBandwidthSampler::is_app_limited() const {
  return is_app_limited_;
}

QuicPacketNumber QuicBandwidthSampler::end_of_app_limited_phase() const {
