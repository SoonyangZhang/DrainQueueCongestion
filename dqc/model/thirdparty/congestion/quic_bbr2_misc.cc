...
void MinRttFilter::Update(TimeDelta sample_rtt, QuicTime now) {
...
void MinRttFilter::ForceUpdate(TimeDelta sample_rtt, QuicTime now) {
  min_rtt_ = sample_rtt;
  min_rtt_timestamp_ = now;
}
    ...
    : params_(params),
      bandwidth_sampler_([](QuicRoundTripCount max_height_tracker_window_length,
        ...
        if (old_sampler != nullptr) {
          return QuicBandwidthSampler(*old_sampler);
        }
        return QuicBandwidthSampler(/*unacked_packet_map=*/nullptr,
