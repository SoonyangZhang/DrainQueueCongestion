...
namespace dqc{
class RttStats;
class BaliaSender : public SendAlgorithmInterface {
 public:
  BaliaSender(const ProtoClock* clock,
                      const RttStats* rtt_stats,
                      QuicPacketCount initial_tcp_congestion_window,
                      QuicPacketCount max_congestion_window,
                      QuicConnectionStats* stats);
  BaliaSender(const BaliaSender&) = delete;
  BaliaSender& operator=(const BaliaSender&) = delete;
  ~BaliaSender() override;
 ...
 private:
  ...
  std::list<BaliaSender*> other_ccs_;
  uint64_t ai_{0};
  float md_{0};
  bool first_{true};
};
