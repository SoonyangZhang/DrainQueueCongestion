...
#include "logging.h"
//this code if modified from 
//https://github.com/dlxg/Linux-NetSpeed/blob/master/bbr/tcp_tsunami.c
namespace dqc{
class RttStats;
// TsunamiSender implements BBR congestion control algorithm.  BBR aims to estimate
...
class TsunamiSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const TsunamiSender& sender);
  ...
  TsunamiSender(ProtoTime now,
            const RttStats* rtt_stats,
            const UnackedPacketMap* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random);
  TsunamiSender(const TsunamiSender&) = delete;
  TsunamiSender& operator=(const TsunamiSender&) = delete;
  ~TsunamiSender() override;
 ...
 std::ostream& operator<<(std::ostream& os,
                                             const TsunamiSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const TsunamiSender::DebugState& state);
}
