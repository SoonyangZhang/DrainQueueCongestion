...
namespace dqc{
class RttStats;
// BbrRandSender implements BBR congestion control algorithm.  BBR aims to estimate
...
class BbrRandSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const BbrRandSender& sender);
  ...
  BbrRandSender(ProtoTime now,
            const RttStats* rtt_stats,
            const UnackedPacketMap* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random);
  BbrRandSender(const BbrRandSender&) = delete;
  BbrRandSender& operator=(const BbrRandSender&) = delete;
  ~BbrRandSender() override;
 ...
 private:
  ...
  void SetProbeBwState(int state);
  ...
  HumanoidCountType humanoid_counter_;
  ProtoTime enter_probe_time_{ProtoTime::Zero()};
  int drain_rtt_times_{0};
  int drain_rtt_len_{0};
};

 std::ostream& operator<<(std::ostream& os,
                                             const BbrRandSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const BbrRandSender::DebugState& state);
}
