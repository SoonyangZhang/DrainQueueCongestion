...
HybridSlowStart::HybridSlowStart()
    : started_(false),
      ...
      rtt_sample_count_(0),
      current_min_rtt_(TimeDelta::Zero()) {}
...
void HybridSlowStart::StartReceiveRound(QuicPacketNumber last_sent) {
  DLOG(INFO) << "Reset hybrid slow start @" << last_sent;
...
bool HybridSlowStart::ShouldExitSlowStart(TimeDelta latest_rtt,
                                          TimeDelta min_rtt,
  ...
  if (rtt_sample_count_ == kHybridStartMinSamples) {
    ...
    TimeDelta min_rtt_increase_threshold =
        TimeDelta::FromMicroseconds(std::max(
