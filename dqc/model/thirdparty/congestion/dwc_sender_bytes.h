...
//pdf: Dynamic Window Coupling for Multipath Congestion Control
namespace dqc{
class RttStats;
class DwcSender : public SendAlgorithmInterface {
 public:
  DwcSender(const ProtoClock* clock,
                      const RttStats* rtt_stats,
                      bool reno,
                      QuicPacketCount initial_tcp_congestion_window,
                      QuicPacketCount max_congestion_window,
                      QuicConnectionStats* stats);
  DwcSender(const DwcSender&) = delete;
  DwcSender& operator=(const DwcSender&) = delete;
  ~DwcSender() override;
  ...
  TimeDelta get_smoothed_rtt() const;
  void set_alpha(uint64_t alpha){alpha_=alpha;}
  void DelayInspection();
 protected:
 ...
 private:
  ...
  std::list<DwcSender*> other_ccs_;
  uint64_t alpha_;
  ProtoTime min_rtt_timestamp_;
  TimeDelta min_rtt_;
  ProtoTime max_rtt_timestamp_;
  TimeDelta max_rtt_; //take smooth value
};
}
