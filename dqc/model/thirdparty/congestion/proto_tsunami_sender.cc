...
const float kPacingGain[] = {1.5, 0.75, 1.25, 1.25, 1.25, 1.25, 1.25, 1.25};
...
TsunamiSender::DebugState::DebugState(const TsunamiSender& sender)
    : mode(sender.mode_),
...
TsunamiSender::DebugState::DebugState(const DebugState& state) = default;
...
TsunamiSender::~TsunamiSender() {}
void TsunamiSender::SetInitialCongestionWindowInPackets(
...
bool TsunamiSender::InSlowStart() const {
  return mode_ == STARTUP;
}

void TsunamiSender::OnPacketSent(ProtoTime sent_time,
...
bool TsunamiSender::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth TsunamiSender::PacingRate(QuicByteCount bytes_in_flight) const {
...
QuicBandwidth TsunamiSender::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount TsunamiSender::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount TsunamiSender::GetSlowStartThreshold() const {
  return 0;
}

bool TsunamiSender::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool TsunamiSender::ShouldSendProbingPacket() const {
  if (pacing_gain_ <= 1) {
    return false;
  }
...
bool TsunamiSender::IsPipeSufficientlyFull() const {
...
void TsunamiSender::AdjustNetworkParameters(QuicBandwidth bandwidth,
                                        TimeDelta rtt,
...
void TsunamiSender::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType TsunamiSender::GetCongestionControlType() const {
  return kTsunami;
}

TimeDelta TsunamiSender::GetMinRtt() const {
  return !min_rtt_.IsZero() ? min_rtt_ : rtt_stats_->initial_rtt();
}

QuicByteCount TsunamiSender::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount TsunamiSender::ProbeRttCongestionWindow() const {
...
void TsunamiSender::EnterStartupMode(ProtoTime now) {
  /*if (stats_) {
...
void TsunamiSender::EnterProbeBandwidthMode(ProtoTime now) {
  mode_ = PROBE_BW;
...
void TsunamiSender::DiscardLostPackets(const LostPacketVector& lost_packets) {
...
bool TsunamiSender::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool TsunamiSender::UpdateBandwidthAndMinRtt(
    ProtoTime now,
    const AckedPacketVector& acked_packets) {
...
bool TsunamiSender::ShouldExtendMinRttExpiry() const {
...
void TsunamiSender::UpdateGainCyclePhase(ProtoTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
...
void TsunamiSender::CheckIfFullBandwidthReached() {
...
void TsunamiSender::MaybeExitStartupOrDrain(ProtoTime now) {
...
void TsunamiSender::OnExitStartup(ProtoTime now) {
...
void TsunamiSender::MaybeEnterOrExitProbeRtt(ProtoTime now,
                                         bool is_round_start,
                                         bool min_rtt_expired) {
...
void TsunamiSender::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
QuicByteCount TsunamiSender::UpdateAckAggregationBytes(
    ProtoTime ack_time,
...
void TsunamiSender::CalculatePacingRate() {
...
void TsunamiSender::CalculateCongestionWindow(QuicByteCount bytes_acked,
...
void TsunamiSender::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string TsunamiSender::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void TsunamiSender::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
TsunamiSender::DebugState TsunamiSender::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(TsunamiSender::Mode mode) {
  switch (mode) {
    case TsunamiSender::STARTUP:
      return "STARTUP";
    case TsunamiSender::DRAIN:
      return "DRAIN";
    case TsunamiSender::PROBE_BW:
      return "PROBE_BW";
    case TsunamiSender::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const TsunamiSender::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const TsunamiSender::DebugState& state) {
  ...
  if (state.mode == TsunamiSender::STARTUP) {
