...
const float kPacingGain[] = {1.5, 0.5, 1.5, 0.5, 1.5, 0.5, 1.5, 0.5};
...
HighSeedRailSender::DebugState::DebugState(const HighSeedRailSender& sender)
    : mode(sender.mode_),
...
HighSeedRailSender::DebugState::DebugState(const DebugState& state) = default;

HighSeedRailSender::HighSeedRailSender(ProtoTime now,
...
HighSeedRailSender::~HighSeedRailSender() {}
void HighSeedRailSender::SetInitialCongestionWindowInPackets(
...
bool HighSeedRailSender::InSlowStart() const {
  return mode_ == STARTUP;
}

void HighSeedRailSender::OnPacketSent(ProtoTime sent_time,
...
bool HighSeedRailSender::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth HighSeedRailSender::PacingRate(QuicByteCount bytes_in_flight) const {
...
QuicBandwidth HighSeedRailSender::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount HighSeedRailSender::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount HighSeedRailSender::GetSlowStartThreshold() const {
  return 0;
}

bool HighSeedRailSender::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool HighSeedRailSender::ShouldSendProbingPacket() const {
  if (pacing_gain_ <= 1) {
    return false;
  }
...
bool HighSeedRailSender::IsPipeSufficientlyFull() const {
...
void HighSeedRailSender::AdjustNetworkParameters(QuicBandwidth bandwidth,
                                        TimeDelta rtt,
...
void HighSeedRailSender::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType HighSeedRailSender::GetCongestionControlType() const {
  return kHighSpeedRail;
}

TimeDelta HighSeedRailSender::GetMinRtt() const {
  return !min_rtt_.IsZero() ? min_rtt_ : rtt_stats_->initial_rtt();
}

QuicByteCount HighSeedRailSender::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount HighSeedRailSender::ProbeRttCongestionWindow() const {
...
void HighSeedRailSender::EnterStartupMode(ProtoTime now) {
  /*if (stats_) {
...
void HighSeedRailSender::EnterProbeBandwidthMode(ProtoTime now) {
  mode_ = PROBE_BW;
...
void HighSeedRailSender::DiscardLostPackets(const LostPacketVector& lost_packets) {
...
bool HighSeedRailSender::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool HighSeedRailSender::UpdateBandwidthAndMinRtt(
    ProtoTime now,
    const AckedPacketVector& acked_packets) {
...
bool HighSeedRailSender::ShouldExtendMinRttExpiry() const {
...
void HighSeedRailSender::UpdateGainCyclePhase(ProtoTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
...
void HighSeedRailSender::CheckIfFullBandwidthReached() {
...
void HighSeedRailSender::MaybeExitStartupOrDrain(ProtoTime now) {
...
void HighSeedRailSender::OnExitStartup(ProtoTime now) {
...
void HighSeedRailSender::MaybeEnterOrExitProbeRtt(ProtoTime now,
                                         bool is_round_start,
                                         bool min_rtt_expired) {
...
void HighSeedRailSender::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
QuicByteCount HighSeedRailSender::UpdateAckAggregationBytes(
    ProtoTime ack_time,
...
void HighSeedRailSender::CalculatePacingRate() {
...
void HighSeedRailSender::CalculateCongestionWindow(QuicByteCount bytes_acked,
...
void HighSeedRailSender::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string HighSeedRailSender::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void HighSeedRailSender::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
HighSeedRailSender::DebugState HighSeedRailSender::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(HighSeedRailSender::Mode mode) {
  switch (mode) {
    case HighSeedRailSender::STARTUP:
      return "STARTUP";
    case HighSeedRailSender::DRAIN:
      return "DRAIN";
    case HighSeedRailSender::PROBE_BW:
      return "PROBE_BW";
    case HighSeedRailSender::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const HighSeedRailSender::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const HighSeedRailSender::DebugState& state) {
  ...
  if (state.mode == HighSeedRailSender::STARTUP) {
