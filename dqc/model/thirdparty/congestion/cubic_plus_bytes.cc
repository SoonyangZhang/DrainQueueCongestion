...
CubicPlusBytes::CubicPlusBytes(const ProtoClock* clock)
    : clock_(clock),
      num_connections_(kDefaultNumConnections),
      epoch_(ProtoTime::Zero()) {
  ResetCubicState();
}

void CubicPlusBytes::SetNumConnections(int num_connections) {
  num_connections_ = num_connections;
}

float CubicPlusBytes::Alpha() const {
...
float CubicPlusBytes::Beta() const {
...
float CubicPlusBytes::BetaLastMax() const {
...
void CubicPlusBytes::ResetCubicState() {
  epoch_ = ProtoTime::Zero();             // Reset time.
...
void CubicPlusBytes::UpdateCongestionWindowEstimate(QuicByteCount estimated_congestion_window,
		QuicByteCount max_congestion_window){
	ResetCubicState();
	estimated_tcp_congestion_window_=estimated_congestion_window;
	last_target_congestion_window_=max_congestion_window;
}
void CubicPlusBytes::OnApplicationLimited() {
...
QuicByteCount CubicPlusBytes::CongestionWindowAfterPacketLoss(
...
QuicByteCount CubicPlusBytes::CongestionWindowAfterAck(
  ...
  DLOG(INFO) << "Final target congestion_window: "
                << target_congestion_window;
