...
namespace dqc{
class RttStats;
// BbrPlusSender implements BBR congestion control algorithm.  BBR aims to estimate
...
class BbrPlusSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const BbrPlusSender& sender);
  ...
  BbrPlusSender(ProtoTime now,
            const RttStats* rtt_stats,
            const UnackedPacketMap* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random);
  BbrPlusSender(const BbrPlusSender&) = delete;
  BbrPlusSender& operator=(const BbrPlusSender&) = delete;
  ~BbrPlusSender() override;
 ...
 private:
  ...
  void SetCycleIdx(int cycle_idx);
 ...
 std::ostream& operator<<(std::ostream& os,
                                             const BbrPlusSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const BbrPlusSender::DebugState& state);
}
