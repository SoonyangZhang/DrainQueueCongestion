...
#include "proto_circular_deque.h"
...
namespace dqc {
#define QuicUnackedPacketMap  UnackedPacketMap
// A subset of QuicBandwidthSampler::ConnectionStateOnSentPacket which is returned
// to the caller when the packet is acked or lost.
struct QuicSendTimeState {
  QuicSendTimeState()
      : is_valid(false),
  ...
  QuicSendTimeState(bool is_app_limited,
  ...
  QuicSendTimeState(const QuicSendTimeState& other) = default;

  friend std::ostream& operator<<(std::ostream& os,
                                                      const QuicSendTimeState& s);
...
struct QuicBandwidthSample {
  ...
  QuicSendTimeState state_at_send;

  QuicBandwidthSample()
      : bandwidth(QuicBandwidth::Zero()), rtt(TimeDelta::Zero()) {}
};

// MaxAckHeightTracker is part of the QuicBandwidthSampler. It is called after every
...
class  MaxAckHeightTracker {
 ...
 private:
  ...
  ProtoTime aggregation_epoch_start_time_ = ProtoTime::Zero();
  ...
  double ack_aggregation_bandwidth_threshold_ =1.0;
...
class  QuicBandwidthSamplerInterface {
 public:
  virtual ~QuicBandwidthSamplerInterface() {}
  ...
  struct CongestionEventSample {
    ...
    // TimeDelta::Infinite() if no samples are available.
    TimeDelta sample_rtt = TimeDelta::Infinite();
...
// QuicBandwidthSampler keeps track of sent and acknowledged packets and outputs a
...
//
// For that purpose, QuicBandwidthSampler always keeps track of the most recently
...
class QuicBandwidthSampler : public QuicBandwidthSamplerInterface {
 public:
  QuicBandwidthSampler(const QuicUnackedPacketMap* unacked_packet_map,
  ...
  QuicBandwidthSampler(const QuicBandwidthSampler& other);
  ~QuicBandwidthSampler() override;
  ...
  struct QUIC_NO_EXPORT AckPoint {
    ProtoTime ack_time = ProtoTime::Zero();
  ...
  class QUIC_NO_EXPORT RecentAckPoints {
   public:
    void Update(ProtoTime ack_time, QuicByteCount total_bytes_acked) {
 ...
 private:
  ...
  struct ConnectionStateOnSentPacket {
    ...
    ConnectionStateOnSentPacket(ProtoTime sent_time,
                                QuicByteCount size,
                                QuicByteCount bytes_in_flight,
                                const QuicBandwidthSampler& sampler)
        : sent_time(sent_time),
          size(size),
  ...
  QuicBandwidthSample OnPacketAcknowledged(ProtoTime ack_time,
                                       QuicPacketNumber packet_number);

  QuicSendTimeState OnPacketLost(QuicPacketNumber packet_number,
                             QuicPacketLength bytes_lost);

  // Copy a subset of the (private) ConnectionStateOnSentPacket to the (public)
  // QuicSendTimeState. Always set send_time_state->is_valid to true.
  ...
  // ProtoTime::Zero() if no valid timestamp is available.
  ...
  QuicBandwidthSample OnPacketAcknowledgedInner(
      ProtoTime ack_time,
      QuicPacketNumber packet_number,
