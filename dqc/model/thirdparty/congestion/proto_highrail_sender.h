...
namespace dqc{
class RttStats;
// HighSeedRailSender implements BBR congestion control algorithm.  BBR aims to estimate
...
class HighSeedRailSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const HighSeedRailSender& sender);
  ...
  HighSeedRailSender(ProtoTime now,
            const RttStats* rtt_stats,
            const UnackedPacketMap* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random);
  HighSeedRailSender(const HighSeedRailSender&) = delete;
  HighSeedRailSender& operator=(const HighSeedRailSender&) = delete;
  ~HighSeedRailSender() override;
 ...
 std::ostream& operator<<(std::ostream& os,
                                             const HighSeedRailSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const HighSeedRailSender::DebugState& state);
}
