...
class DelayBbrSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const DelayBbrSender& sender);
  ...
  DelayBbrSender(ProtoTime now,
            ...
            Random* random);
  DelayBbrSender(const DelayBbrSender&) = delete;
  DelayBbrSender& operator=(const DelayBbrSender&) = delete;
  ~DelayBbrSender() override;
 ...
 private:
  ...
  bool active_drain_{false};
  bool probe_active_drain_{false};
  TimeDelta srtt_{TimeDelta::Zero()};
  TimeDelta base_line_rtt_{TimeDelta::Infinite()};
  QuicPacketNumber seq_sent_when_backoff_{1};
  QuicByteCount bdp_{0};
};

 std::ostream& operator<<(std::ostream& os,
                                             const DelayBbrSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const DelayBbrSender::DebugState& state);
}
