...
class CoupleBbrSender : public SendAlgorithmInterface {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const CoupleBbrSender& sender);
  ...
  CoupleBbrSender(ProtoTime now,
            ...
            Random* random);
  CoupleBbrSender(const CoupleBbrSender&) = delete;
  CoupleBbrSender& operator=(const CoupleBbrSender&) = delete;
  ~CoupleBbrSender() override;
  ...
  //the method to implement couple congestion control algorithm.
 ...
 std::ostream& operator<<(std::ostream& os,
                                             const CoupleBbrSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const CoupleBbrSender::DebugState& state);
}
