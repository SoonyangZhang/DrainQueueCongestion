...
std::string SocketAddress::ToString() const {
  ...
  std::string str=HostAsURIString()+":" +std::to_string(port());
  return str;
}
...
std::string SocketAddress::ToSensitiveString() const {
  //char buf[1024];
  //rtc::SimpleStringBuilder sb(buf);
  std::string str=HostAsSensitiveURIString() +":" +std::to_string(port());
  return str;
}
...
void SocketAddress::ToSockAddr(sockaddr_in* saddr) const {
  ...
  saddr->sin_port = basic::HostToNet16(port_);
  if (IPIsAny(ip_)) {
...
bool SocketAddress::FromSockAddr(const sockaddr_in& saddr) {
  if (saddr.sin_family != AF_INET)
    return false;
  SetIP(basic::NetToHost32(saddr.sin_addr.s_addr));
  SetPort(basic::NetToHost16(saddr.sin_port));
  literal_ = false;
  return true;
}
