...
uint32_t IPAddress::v4AddressAsHostOrderInteger() const {
  if (family_ == AF_INET) {
    return basic::NetToHost32(u_.ip4.s_addr);
  } else {
    return 0;
  }
}
...
bool IPAddress::operator<(const IPAddress& other) const {
  ...
  switch (family_) {
    case AF_INET: {
      return basic::NetToHost32(u_.ip4.s_addr) <
             basic::NetToHost32(other.u_.ip4.s_addr);
    }
    case AF_INET6: {
...
std::string IPAddress::ToString() const {
  ...
  if (!rtc_inet_ntop(family_, src, buf, sizeof(buf))) {
    return std::string();
  }
...
bool IPFromString(const std::string& str, IPAddress* out) {
  ...
  in_addr addr;
  if (rtc_inet_pton(AF_INET, str.c_str(), &addr) == 0) {
    in6_addr addr6;
    if (rtc_inet_pton(AF_INET6, str.c_str(), &addr6) == 0) {
      *out = IPAddress();
      return false;
    }
    *out = IPAddress(addr6);
...
IPAddress TruncateIP(const IPAddress& ip, int length) {
  ...
  if (ip.family() == AF_INET) {
    ...
    uint32_t host_order_ip = basic::NetToHost32(ip.ipv4_address().s_addr);
    in_addr masked;
    masked.s_addr = basic::HostToNet32(host_order_ip & mask);
    ...
    for (int i = 0; i < 4; ++i) {
      if (i == position) {
        uint32_t host_order_inner = basic::NetToHost32(v6_as_ints[i]);
        v6_as_ints[i] = basic::HostToNet32(host_order_inner & inner_mask);
      } else if (i > position) {
        v6_as_ints[i] = 0;
      }
    }
...
int CountIPMaskBits(IPAddress mask) {
  ...
  switch (mask.family()) {
    case AF_INET: {
      word_to_count = basic::NetToHost32(mask.ipv4_address().s_addr);
      break;
    }
    case AF_INET6: {
      ...
      if (i < 4) {
        word_to_count = basic::NetToHost32(v6_as_ints[i]);
      }
      bits = (i * 32);
      break;
    }
