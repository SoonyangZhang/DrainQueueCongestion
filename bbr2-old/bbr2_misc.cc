...
MinRttFilter::MinRttFilter(TimeDelta initial_min_rtt,
                           ProtoTime initial_min_rtt_timestamp)
    : min_rtt_(initial_min_rtt),
      min_rtt_timestamp_(initial_min_rtt_timestamp) {}

void MinRttFilter::Update(TimeDelta sample_rtt, ProtoTime now) {
  if (sample_rtt < min_rtt_ || min_rtt_timestamp_ == ProtoTime::Zero()) {
    min_rtt_ = sample_rtt;
    min_rtt_timestamp_ = now;
  }
}

void MinRttFilter::ForceUpdate(TimeDelta sample_rtt, ProtoTime now) {
  min_rtt_ = sample_rtt;
  min_rtt_timestamp_ = now;
}
...
void Bbr2NetworkModel::OnPacketSent(ProtoTime sent_time,
